...
class UserServiceTest {
    ...
    @Test
    fun saveWithSuccess() {
        Mockito.`when`(userRepository.save(user.copy(id = null))).thenReturn(this.user)

        userService.saveUser(this.userRequestDTO)

        Mockito.verify(userRepository, Mockito.times(1)).save(user.copy(id = null))
    }
    ...
    @Test
    fun getByCpfWithErrorTest() {
        Mockito.`when`(userRepository.findByCpf(user.cpf, true)).thenReturn(null)

        val exception = assertThrows<ResourceNotFoundBankException> { userService.getByCpf(user.cpf, true) }
    ...
    @Test
    fun getByCpfWithSuccessTest() {
        Mockito.`when`(userRepository.findByCpf(user.cpf, true)).thenReturn(user)

        val userResponse = userService.getByCpf(user.cpf, true)
        Assert.assertEquals(this.userResponseDTO, userResponse)
    ...
    fun existsByCpfTest() {
        ...
        val exception = assertThrows<DuplicatedResourceBankException> { userService.validateFields(userRequestDTO) }
